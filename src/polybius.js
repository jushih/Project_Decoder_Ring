...
const polybiusModule = (function () {
  ...
  function polybius(input, encode = true) {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      // encode
      if (encode === true) {
          word += polyEncode[cleanInput[i]]
      }
      
      // decode
      else {
           // if odd length input is given, cipher does not work. return false 
        if ( (input.replace(/\s/g, "").length) % 2 === 1) {
        return false
    }
        
        // check if there is a space, if so, append space and return to start of loop
        if (input[i] === " ") {
          word += polyDecode[cleanInput[i]]
        }
        else {
        // if input is a number, check two digits and look up the decoded letter. skip ahead two indices and keep looping
        let num = input[i] + input[i+1]
        console.log(polyDecode[num])
        word += polyDecode[num]
          i++
        }
      }
